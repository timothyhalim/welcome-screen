...
class Array_Knob(Knob):
    ...
    def isAnimated(self, index, view):
    ...
    def setExpression(self, expression, channel=-1, view=None):
...
class CascadingEnumeration_Knob(Enumeration_Knob):
    ...
    to the saved scripts (so that the numerical values can change). To create cascading submenus simply use the forward slash ( '/' ) e.g. menu/item1"""
...
class Format_Knob(Knob):
    ...
    def toScript(self, quote, context=current):
...
class MenuItem(object):
    ...
    def setEnabled(self, enabled, recursive):
...
class MultiView_Knob(Knob):
    ...
    def value(self, quote, context=current):
        """toScript(quote, context=current) -> string.
...
class Panel(object):
    ...
    def addEnumerationPulldown(self, name, value):
    ...
    def addPasswordInput(self, name, value):
...
class SceneView_Knob(Unsigned_Knob):
    ...
    def setAllItems(self, items, autoSelect):
...
def afterBackgroundFrameRender():
    """None"""
    pass
    

def afterBackgroundRender():
    """None"""
    pass
...
def createNode(node, args, inpanel):
...
def createScenefileBrowser(fileName, nodeName):
...
def display(s, node, title, width):
...
def executeBackgroundNuke(exe_path, nodes, frameRange, views, limits, continueOnError, =, False, flipbookToRun, =, flipbookOptions, =, {}):
...
def numvalue(knob, default=infinity):
...
def removeAutoSaveDeleteFilter():
    """Remove a previously-added callback with the same arguments."""
    pass
    

def removeAutoSaveFilter():
    """Remove a previously-added callback with the same arguments."""
    pass
    

def removeAutoSaveRestoreFilter():
    """Remove a previously-added callback with the same arguments."""
    pass
